...
@property (nonatomic, strong)  NSArray  * stringKeyArr;
...
//    NSMutableDictionary * mdic = [NSMutableDictionary dictionary];
    NSDictionary * mdic = [NSDictionary dictionaryWithContentsOfFile:[NSString stringWithFormat:@"%@/Localizable.strings",self.txtShowPath.placeholderString]];
    
    
    self.stringDict = mdic;
    self.stringKeyArr = mdic.allKeys;
    self.index = 0;
    if (self.stringKeyArr.count > 0) {
        NSString * key = self.stringKeyArr[self.index];
        [self replaceText:self.stringKeyArr[self.index] value:self.stringDict[key]];
    }
}

-(void)replaceText:(NSString *)text value:(NSString *)value{

    if (self.index > self.stringKeyArr.count-1) {
        return;
    }
...
//            NSString * sh2 = [NSString stringWithFormat:@"sed -i '' -e s/'%@'/'%@'/g `grep %@ -rl %@`",text,dst,text,self.txtShowOutPath.placeholderString];
            ...
            NSString * sh2 = [NSString stringWithFormat:@"sed -i '' -e s/'%@'/'%@'/g %@/Localizable.strings",value,dst,self.txtShowOutPath.placeholderString];
            [weak_self cmd:sh2];
        }
